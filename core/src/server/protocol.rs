...
mod msgpack_tests {
    ...
    #[test]
    fn test_msgpack_list_projects() {
        // 测试 Rust 端编码的格式
        let msg = ClientMessage::ListProjects;
        let encoded = rmp_serde::to_vec(&msg).unwrap();
        println!("Rust encoded ListProjects: {:?}", encoded);
        println!("Rust encoded hex: {:02x?}", encoded);
        
        // 尝试解码
        let decoded: ClientMessage = rmp_serde::from_slice(&encoded).unwrap();
        println!("Decoded: {:?}", decoded);
    }
}

    #[test]
    fn test_msgpack_dict_format() {
        // 模拟 Swift 端发送的字典格式 {"type": "list_projects"}
        // MessagePack map 格式
        let dict_bytes: Vec<u8> = vec![
            0x81,  // fixmap with 1 element
            0xa4, 0x74, 0x79, 0x70, 0x65,  // "type" (fixstr len 4)
            0xad, 0x6c, 0x69, 0x73, 0x74, 0x5f, 0x70, 0x72, 0x6f, 0x6a, 0x65, 0x63, 0x74, 0x73  // "list_projects" (fixstr len 13)
        ];
        
        println!("Dict format bytes: {:02x?}", dict_bytes);
        
        // 尝试解码
        let result: Result<ClientMessage, _> = rmp_serde::from_slice(&dict_bytes);
        match result {
            Ok(msg) => println!("Decoded successfully: {:?}", msg),
