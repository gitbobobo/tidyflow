...
async fn test_protocol_v1() {
    ...
    println!("[Test 1] Hello message...");
    {
        ...
        let msg = recv_json_skip_output(&mut read, "hello")
            .await
    ...
    println!("[Test 2] Ping/Pong...");
    {
        ...
        write
            ...
            .await
            .unwrap();
        let msg = recv_json_skip_output(&mut read, "pong")
            .await
            .expect("No pong");
    ...
    println!("[Test 3] List Projects...");
    {
        ...
        write
            .send(Message::Text(json!({"type": "list_projects"}).to_string()))
            .await
            .unwrap();
        let msg = recv_json_skip_output(&mut read, "projects")
            .await
    ...
    println!("[Test 4] List Workspaces (error case)...");
    {
        ...
        write
            .send(Message::Text(
                json!({"type": "list_workspaces", "project": "nonexistent_xyz"}).to_string(),
            ))
            .await
            .unwrap();
        let msg = recv_json_skip_output(&mut read, "error")
            .await
    ...
    println!("[Test 5] Spawn Terminal with CWD...");
    {
        ...
        write
            .send(Message::Text(
                json!({"type": "spawn_terminal", "cwd": "/tmp"}).to_string(),
            ))
            .await
            .unwrap();
        let msg = recv_json_skip_output(&mut read, "terminal_spawned")
            .await
        ...
        println!(
            ...
            &msg["session_id"].as_str().unwrap()[..8]
        );
    }
    ...
    println!("[Test 6] Spawn Terminal (invalid path)...");
    {
        ...
        write
            .send(Message::Text(
                json!({"type": "spawn_terminal", "cwd": "/nonexistent/path/xyz"}).to_string(),
            ))
            .await
            .unwrap();
        let msg = recv_json_skip_output(&mut read, "error")
    ...
    println!("[Test 7] Terminal I/O...");
    {
        ...
        write
            .send(Message::Text(
                json!({"type": "resize", "cols": 80, "rows": 24}).to_string(),
            ))
            .await
            .unwrap();
        ...
        write
            .send(Message::Text(
                json!({"type": "input", "data_b64": data_b64}).to_string(),
            ))
            .await
            .unwrap();
    ...
    {
        let (mut write, mut read) = connect_to_server().await.expect("Failed to connect");
        let hello = recv_json_skip_output(&mut read, "hello")
            .await
            .expect("No hello");
        let session_id = hello["session_id"].as_str().unwrap().to_string();

        write
            .send(Message::Text(json!({"type": "kill_terminal"}).to_string()))
            .await
            .unwrap();
        let msg = recv_json_skip_output(&mut read, "terminal_killed")
            .await
