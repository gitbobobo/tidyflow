...
(function () {
  ...
  function refreshExplorer() {
    ...
    if (!currentProject || !currentWorkspace) {
      explorerTreeEl.innerHTML =
        '<div class="file-empty">No workspace selected</div>';
      return;
    }
  ...
  function renderExplorerTree(path, items) {
    ...
    if (path === ".") {
      ...
      if (items.length === 0) {
        explorerTreeEl.innerHTML =
          '<div class="file-empty">Empty directory</div>';
        return;
      }
      items.forEach((item) => {
  ...
  function performSearch(query) {
    ...
    if (!query.trim()) {
      resultsEl.innerHTML =
        '<div class="search-empty">Enter a search term</div>';
      return;
    }

    const lowerQuery = query.toLowerCase();
    const matches = allFilePaths.filter((p) =>
    ...
    matches.slice(0, 50).forEach((path) => {
      ...
      name.innerHTML =
        ...
        '<span class="match">' +
        path.substring(idx, idx + query.length) +
        "</span>" +
  ...
  function renderGitStatus(repoRoot, items) {
    ...
    items.forEach((item) => {
      ...
      if (item.code === "M") status.classList.add("modified");
        ...
        status.classList.add("untracked");
      else if (item.code === "R" || item.code === "C")
        status.classList.add("renamed");
  ...
  function createTerminalTab(termId, cwd, project, workspace) {
    ...
    const term = new Terminal({
      ...
      fontSize: 14,
      fontFamily:
        '"MesloLGS NF", "Meslo LG M DZ", "FiraCode Nerd Font", "JetBrains Mono", Menlo, Monaco, "Courier New", monospace',
    ...
    // Fix for macOS IME duplication: track composition state
  ...
  function highlightLine(view, lineNumber, highlightMs = 2000) {
    ...
    const { EditorView, Decoration, StateEffect, StateField } =
      window.CodeMirror;
    ...
    try {
      ...
      if (tabInfo && tabInfo.term) {
        ...
        setTimeout(() => {
          ...
          // 回放暂存的 output
          const itemsToReplay = pendingOutputBuffer.filter(
            (item) => item.termId === sessionId,
          );
          pendingOutputBuffer = pendingOutputBuffer.filter(
            (item) => item.termId !== sessionId,
          );
    ...
    try {
      ...
      switch (msg.type) {
        ...
        case "output": {
          ...
          if (
            termId &&
            !terminalSessions.has(termId) &&
            termId === defaultServerTerminalId
          ) {
        ...
        case "selected_workspace": {
          ...
          const tabInfo = createTerminalTab(
            msg.session_id,
            msg.root,
            msg.project,
            msg.workspace,
        ...
        case "term_created": {
          ...
          const pendingTabId = pendingTerminalSpawn
            ? pendingTerminalSpawn.tabId
            : null;
          ...
          if (tabInfo.term) {
            tabInfo.term.writeln(
              "\x1b[32m[New Terminal: " +
                (msg.workspace || "default") +
                "]\x1b[0m",
            );
        ...
        case "file_list_result":
          if (
            msg.project === currentProject &&
            msg.workspace === currentWorkspace
          ) {
            renderExplorerTree(msg.path, msg.items);
          }
          notifySwift("file_list", {
            project: msg.project,
            workspace: msg.workspace,
            items: msg.items,
          });
            ...
            try {
              ...
              if (tabInfo) {
                ...
                if (
                  ...
                  pendingLineNavigation.filePath === msg.path
                ) {
        ...
        case "file_index_result": {
          ...
          if (
            ...
            window.tidyflowPalette.onFileIndexReady
          ) {
            window.tidyflowPalette.onFileIndexReady(wsKey);
          }
          notifySwift("file_index", {
            project: msg.project,
            workspace: msg.workspace,
  ...
  function handleNativeEvent(type, payload) {
    ...
    switch (type) {
      ...
      case "save_file": {
        ...
        if (workspaceTabs.has(wsKey)) {
          ...
          if (tabSet.tabs.has(tabId)) {
            ...
            postToNative("save_error", {
              path,
              message: "Editor tab not found",
            });
          }
        } else {
      ...
      case "terminal_spawn": {
        ...
        if (!transport || !transport.isConnected) {
          console.log(
            "[NativeBridge] WebSocket not connected, connecting first...",
          );
          connect();
          ...
          setTimeout(() => {
            if (transport && transport.isConnected) {
              ...
              console.log(
                "[NativeBridge] Sending select_workspace:",
                project,
                workspace,
              );
              sendControlMessage({
      ...
      case "editor_reveal_line": {
        ...
        console.log(
          "[NativeBridge] editor_reveal_line:",
          path,
          line,
          highlightMs,
        );
        ...
        if (!path || !line) {
          console.error(
            "[NativeBridge] editor_reveal_line missing required fields",
          );
          return;
        }
    ...
    if (terminalTab) {
      ...
      setTimeout(() => {
        ...
        if (terminalTab.term) {
          ...
          sendResize(
            terminalTab.termId,
            terminalTab.term.cols,
            terminalTab.term.rows,
          );
        }
      }, 50);
    }
  ...
  function initNativeBridge() {
    if (window.tidyflowNative) {
      ...
      console.log(
        "[NativeBridge] Native bridge not available (running in browser?)",
      );
    }
  }
