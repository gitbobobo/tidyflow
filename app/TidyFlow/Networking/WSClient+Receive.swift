        ...
        case ("terminal", "output_batch"):
            terminalReducerQueue.async { [weak self] in
                ...
                do {
                    ...
                    TFLog.ws.error("Terminal output_batch typed decode failed: \(error.localizedDescription, privacy: .public)")
                }
            }
            return true
            ...
            aiReducerQueue.async { [weak self] in
                ...
                do {
                    ...
                    guard let typed = AISessionMessagesUpdateV2.from(json: json) else {
                        TFLog.ws.warning("Failed to convert ai_session_messages_update payload")
                        return
                    }
                    DispatchQueue.main.async { [weak self] in
                        guard let self else { return }
                    ...
                    TFLog.ws.error("AI update typed decode failed: \(error.localizedDescription, privacy: .public)")
                }
            }
            return true
        case ("file", "file_changed"):
            workspaceReducerQueue.async { [weak self] in
                ...
                do {
                    ...
                    TFLog.ws.error("File changed typed decode failed: \(error.localizedDescription, privacy: .public)")
                }
            }
            return true
        case ("git", "git_status_changed"):
            workspaceReducerQueue.async { [weak self] in
                ...
                do {
                    ...
                    TFLog.ws.error("Git status changed typed decode failed: \(error.localizedDescription, privacy: .public)")
                }
            }
            return true
        default:
