...
- [x] 运行 `cd core && cargo clippy -- -D warnings` 无新增警告（2个预存在警告已记录）
- [x] 运行 `cd core && cargo fmt --check` 格式检查通过
...
- [x] 1. 创建 handlers/ 和 terminal/ 目录结构
...
- [x] 2. 拆分 handlers/terminal.rs - 终端相关消息处理
...
- [x] 3. 拆分 handlers/file.rs - 文件操作消息处理
...
- [x] 4. 拆分 handlers/git.rs - Git 消息处理
...
- [x] 5. 拆分 handlers/project.rs - 项目和工作空间管理消息处理
...
- [x] 6. 拆分 handlers/settings.rs - 设置相关消息处理
...
- [x] 7. 拆分 terminal/manager.rs - TerminalManager 和相关方法
...
- [x] 8. 拆分 terminal/session.rs - 终端会话处理逻辑
...
- [x] 9. 更新 ws.rs 和 server/mod.rs - 完成 ws.rs 重构
...
- [x] 11. 拆分 git/utils.rs - 类型定义和辅助函数
...
- [x] 13. 拆分 git/operations.rs - 文件操作函数
...
- [x] 14. 拆分 git/branches.rs - 分支管理函数
...
- [x] 16. 拆分 git/integration.rs - 集成工作流函数
...
- [x] 17. 更新 git_tools.rs 和 server/mod.rs - 完成 git_tools.rs 重构
...
- [x] All "Must NOT Have" absent
  ...
  - [x] 未调整日志级别或格式
- [x] 模块结构符合预期
  - [x] `ws.rs` 大小减小到 929 行（原 3222 行，减少 71%）
  - [x] `git_tools.rs` 已删除
  - [x] `handlers/` 包含 6 个模块（mod, terminal, file, git, project, settings）
  - [x] `git/` 包含 7 个模块（mod, utils, status, operations, branches, commit, integration）
- [x] 代码质量
  - [x] Clippy 无新增警告（2个预存在警告已记录）
  - [x] 代码格式化通过
  - [x] 所有公共类型正确导出
