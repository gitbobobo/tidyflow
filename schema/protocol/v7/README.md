...
新增工作区级文件内容全文搜索 HTTP 只读端点，复用现有文件索引忽略目录策略。
WS 读取动作 `file_content_search` 已移除，Core 返回 `read_via_http_required`。

### HTTP 端点

| 方法 | 端点 | 说明 |
|------|------|------|
| `GET` | `/api/v1/projects/:project/workspaces/:workspace/files/search?query=...&case_sensitive=false` | 工作区文件内容搜索 |

### 域声明（`domains.yaml` file 域）

- `http_read_endpoints`：新增 `files/search` 端点
- `ws_read_via_http_required`：新增 `file_content_search`
- `ws_stream_events`：新增 `file_content_search_result`
- `required_boundary_fields`：`project`、`workspace`

### 搜索约束

- 硬截断上限：1000 条匹配结果 / 5000 文件（`truncated: true`）
- 跳过二进制文件（前 8KB null 字节检测）
- 跳过隐藏目录和 `node_modules`、`target`、`.git` 等
- 结果按 `path ASC → line ASC → column ASC` 排序

### 缓存失效

- `watch_change` 事件触发同工作区 `fileWorkspace` scope 缓存失效
- 客户端按 `(project, workspace)` 二元组隔离搜索缓存

### 验证入口

- `cargo test --manifest-path core/Cargo.toml --test file_content_search` — 文件内容搜索集成测试
- `GlobalSearchSemanticsTests` — Swift 全局搜索语义层测试
