...
- 本地升级脚本在重启前等待端口释放时，必须把“端口已释放”作为启动硬条件；超时应中止自动启动并打印占用 PID，禁止强制继续导致新版本回退动态端口。
...
- iOS 端渲染工具卡片时，避免对超长输出/大对象做 JSON `prettyPrinted`（以及将其写入渲染缓存）；应做“结构摘要 + 截断 + 超大卡片不缓存”兜底，防止加载历史日志时因内存峰值直接闪退（jetsam）。
